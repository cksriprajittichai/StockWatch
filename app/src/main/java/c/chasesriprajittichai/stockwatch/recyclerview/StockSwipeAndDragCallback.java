...
public final class StockSwipeAndDragCallback extends ItemTouchHelper.SimpleCallback {
    ...
    private final Map<String, Integer> mtickerToIndexMap;
    ...
    public StockSwipeAndDragCallback(final HomeActivity homeActivity, final RecyclerAdapter recyclerAdapter,
        ...
        mstocks = stocks;
        mtickerToIndexMap = tickerToIndexMap;
     ...
     * @param direction  The direction to which the ViewHolder is swiped. Should always
     *                   be {@link ItemTouchHelper#LEFT}.
     */
    @Override
    public void onSwiped(final RecyclerView.ViewHolder viewHolder, final int direction) {
        final int position = viewHolder.getAdapterPosition();
        mtickerToIndexMap.remove(mstocks.get(position).getTicker()); // Do before removal from mstocks
        mrecyclerAdapter.remove(position); // Removes from rview and from mstocks, and updates rview
        mhomeActivity.updateTickerToIndexMap(position);
    }
     ...
     *
     * @param rv         The RecyclerView to which ItemTouchHelper is attached to.
    ...
    /**
     * Called by ItemTouchHelper on RecyclerView's onDraw callback. This method is called
     * constantly while the user is swiping or dragging. This function is where drawing "behind"
     * the selected RecyclerView cell occurs.
     * <p>
     * The red background and garbage icon when swiping
     * a stock left are drawn in this function.
     *
     ...
     *                          {@code ACTION_STATE_DRAG} or {@code ACTION_STATE_SWIPE}.
    ...
    /**
     * Called by ItemTouchHelper on RecyclerView's onDraw callback. This method is called
     * constantly while the user is swiping or dragging. This function is where drawing on
     * the selected RecyclerView cell (includes drawing over other cells) occurs.
     * <p>
     * The white margins around a selected cell that is being dragged are drawn here.
     * The actual cell switching occurs in this function as well. A more traditional drag
     * and drop RecyclerView implementation would execute cell switches in{@link
     * #onMove(RecyclerView, RecyclerView.ViewHolder, RecyclerView.ViewHolder)}. But doing
     * that restricts the precision about when two cells swap. This function swaps cells
     * when the selected cell is more than 50% into the adjacent cell that it is invading.
     * This function takes care of the duties of {@link #onMove(RecyclerView,
     * RecyclerView.ViewHolder, RecyclerView.ViewHolder)}, causing
     * {@link #onMove(RecyclerView, RecyclerView.ViewHolder, RecyclerView.ViewHolder)}
     * to always return true. This function also notifies {@code mrecyclerAdapter} if cells
     * are being dragged, by setting {@link RecyclerAdapter#setDragging(boolean)} to true.
     *
     * @param c                 The canvas which RecyclerView is drawing its children
     * @param rv                The RecyclerView to which ItemTouchHelper is attached to
     * @param viewHolder        The ViewHolder which is being interacted by the User or it was
     ...
     */
    @Override
    public void onChildDrawOver(final Canvas c, final RecyclerView rv, final RecyclerView.ViewHolder viewHolder,
                                final float dX, final float dY, final int actionState, final boolean isCurrentlyActive) {
        final View itemView = viewHolder.itemView;
        ...
        switch (actionState) {
            case ItemTouchHelper.ACTION_STATE_DRAG:
                if (isCurrentlyActive) {
                    /* This function is called multiple times after clearView() is called. Therefore,
                     * any calls to mrecyclerAdapter.setDragging() in clearView() could be erased by
                     * subsequent calls to this function. clearView() will be called after this
                     * method's isCurrentlyActive boolean parameter is set to false. */
                    mrecyclerAdapter.setDragging(true);

                    // y increases as we move further down the screen
                    // Draw left margin
                    whiteMargin.setBounds(itemView.getLeft(), itemView.getTop() + (int) dY,
                            itemView.getLeft() + whiteMarginSize, itemView.getBottom() + (int) dY);
                    whiteMargin.draw(c);

                    // Draw right margin
                    whiteMargin.setBounds(itemView.getRight() - whiteMarginSize, itemView.getTop() + (int) dY,
                            itemView.getRight(), itemView.getBottom() + (int) dY);
                    whiteMargin.draw(c);

                    // Draw top margin
                    whiteMargin.setBounds(itemView.getLeft(), itemView.getTop() + (int) dY,
                            itemView.getRight(), itemView.getTop() + (int) dY + whiteMarginSize);
                    whiteMargin.draw(c);

                    // Draw bottom margin
                    whiteMargin.setBounds(itemView.getLeft(), itemView.getBottom() + (int) dY - whiteMarginSize,
                            itemView.getRight(), itemView.getBottom() + (int) dY);
                    whiteMargin.draw(c);
                    ...
                    if (Math.abs(dY) > itemView.getHeight() / 2) {
                        ...
                        /* These calls replace what would normally be done in onMove(). For this
                         * reason, override onMove() to always return true. */
                        if (dY > 0) {
                            // Invading adjacent cell below
                            if (viewHolder.getAdapterPosition() < mrecyclerAdapter.getItemCount() - 1) {
                                mrecyclerAdapter.swap(viewHolder.getAdapterPosition(), viewHolder.getAdapterPosition() + 1);
                                mhomeActivity.updateTickerToIndexMap();
                            }
                        } else {
